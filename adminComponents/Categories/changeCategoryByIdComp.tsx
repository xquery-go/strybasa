...
import {changeCategoryById} from "@/adminComponents/Categories/CategoryQueries";

export const ChangeCategoryByIdComp = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            const data = await changeCategoryById(token, values);
            setRes(data as Object)
        }
    })
    return (
        <form className={styles.wrapper} onSubmit={formik.handleSubmit}>
            <p className={styles.url}>{`/api/categories/{id}/ (patch)`} </p>
